...
import {
  ...
  IVout,
  MainnetFee,
} from '../../xchain/ada/type';

const Unit = bitcore.Unit;
const AdaLockEventMark = 'ck';
const AdaOPReturnCode = '6a';
const AdaDataIndex = 4;
...
export class ADAChain {
  ...
  async sendLockTxs(
    ...
    if (!memo.startsWith(AdaLockEventMark)) {
  ...
  async sendUnlockTxs(records: AdaUnlock[]): Promise<AdaUnlockResult> {
    ...
    const txSize = transactionWithoutFee.serialize().length / 2;
    const feeRate = await getAdaMainnetFee();
  ...
  isLockTx(txVouts: IVout[]): boolean {
    ...
    const receiveAddr = Buffer.from(secondVoutScriptPubKeyHex.substring(AdaDataIndex), 'hex').toString();

    if (receiveAddr.startsWith(AdaLockEventMark)) {
      logger.debug(
    ...
    return (
      ...
      secondVoutScriptPubKeyHex.startsWith(AdaOPReturnCode) &&
      receiveAddr.startsWith(AdaLockEventMark)
    );
  }
  ...
  async getUnlockTxData(txVins: IVin[], txVouts: IVout[]): Promise<string[]> {
    ...
    for (let i = 0; i < waitVerifyTxVouts.length; i++) {
      ...
      if (voutPubkeyHex.startsWith(AdaOPReturnCode)) {
        logger.debug(`verify op return output data : ${voutPubkeyHex}`);
        return splitTxhash(voutPubkeyHex.substring(AdaDataIndex));
      }
    }
    return [];
  }
...
export async function getAdaMainnetFee(): Promise<MainnetFee> {
  try {
