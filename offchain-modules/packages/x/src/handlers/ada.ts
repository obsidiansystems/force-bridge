...
export class AdaHandler {
  ...
  async watchLockEvents() {
    ...
    while (true) {
      try {
        ...
        await this.adaChain.watchAdaTxEvents(
          ...
          async (adaLockEventData: AdaLockData) => {
            ...
            if (this.role === 'collector') {
              ...
              log(`AdaHandler watchAdaTxEvents save CkbMint successful for ADA tx ${adaLockEventData.txId}.`);
            }
            ...
            await this.db.createAdaLock([
              {
                ...
                status: adaLockEventData.status,
              },
            ]);
            log(`AdaHandler watchAdaTxEvents save ADALock successful for ADA tx ${adaLockEventData.txId}.`);
          },
          ...
          async (ckbTxHash: string) => {
            ...
            log(`AdaHandler watchAdaTxEvents unlockRecords: ${JSON.stringify(records, null, 2)}`);
            if (records.length > 1) {
              throw new Error(
        ...
        log('there is an error occurred during in ada chain watch event', e.toString());
      }
    }
  }
  ...
  async watchUnlockEvents() {
    ...
    log('AdaHandler watchUnlockEvents start');
    while (true) {
      ...
      log(`AdaHandler watchUnlockEvents get ada unlock record and send tx ${JSON.stringify(records, null, 2)}`);
      try {
        ...
        await this.db.saveAdaUnlock(records);
        log(`AdaHandler watchUnlockEvents there is an error occurred during in ada chain send unlock.`, e.toString());
      }
    }
  }
